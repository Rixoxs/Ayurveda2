...
public class ThreadsActivity extends AppCompatActivity implements ThreadsAdapter.DataUpdateAfterDelete {
    ...
    public void addThread(String title,String user_name){
        if(user!=null) {
            mDatabase.child("Solicitudes de foro").push().setValue(new MessageThread(title, user.getUid(), user_name));
            addThreadET.setText("");
        }else{
    ...
    public void deleteThread(String thread_id) {
        mDatabase.child("Solicitudes de foro").child(thread_id).removeValue();
    }

    public void getThreads(){
        mDatabase.child("Solicitudes de foro").addValueEventListener(new ValueEventListener() {
            @Override
